...
coinpunk.controllers.Tx.prototype.defaultFee = '0.1';
...
coinpunk.controllers.Tx.prototype.create = function() {
  ...
  if(address == '')
    ...
    try {
      ...
      errors.push('The provided pro address is not valid.');
    }
  }
  ...
  for(var i=0; i<myAddresses.length;i++) {
    if(myAddresses[i].address == address)
      errors.push('You cannot send to your own procoin wallet.');
  }
  ...
  if(amount == '' || parseFloat(amount) == 0)
    ...
    errors.push('Cannot spend more procoins than you currently have.');
  }
  ...
  self.saveWallet({override: true, address: changeAddress}, function(response) {
    if(response.result == 'error' && response.messages[0] == 'Invalid session key') {
      ...
      $.post('/api/tx/send', {tx: rawtx}, function(resp) {
        coinpunk.database.setSuccessMessage("Sent "+amount+" PCN to "+address+".");
...
coinpunk.controllers.Tx.prototype.calculateFee = function() {
  ...
  $('#fee').text(coinpunk.wallet.calculateFee(amount, address, changeAddress)+' PCN');
...
coinpunk.controllers.Tx.prototype.scanQR = function(event) {
  ...
  qrcode.callback = function(result) {
    ...
    if(uri.protocol() != 'procoin')
      return errorsDiv.removeClass('hidden').text('Not a valid Procoin QR code.');
    
    var address = uri.path();
    if(!address || address == '')
      return errorsDiv.removeClass('hidden').text('No Procoin address found in QR code.');
